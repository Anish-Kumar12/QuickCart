...
const EditProductAdmin = ({ close, data: propsData, fetchProductData }) => {
  const originalData = [...propsData.image];
  const deletionImage = [];
  ...
  const incidentalRemoveImage = async (e) => {
    const newImage = [...data.image];
    for (let i = 0; i < newImage.length; i++) {
      if (newImage[i] !== originalData[i]) {
        await deleteImage(newImage[i]);
      }
    }
  };
  ...
  const handleDeleteImage = async (index) => {
    deletionImage.push(data.image[index]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < deletionImage.length; i++) {
        await deleteImage(deletionImage[i]);
      }
    } catch (error) {
        ...
        <section className="">
          <div className="p-2   bg-white shadow-md flex items-center justify-between">
            ...
            <button onClick={close}>
              <IoClose size={20} onClick={incidentalRemoveImage} />
            </button>
          </div>
